	...
	$.getJSON("/ETS/board/getAttachList",{bno:bno},function(arr){
		...
		$(arr).each(function(i,attach){
			...
			if(attach.fileType){ // 이미지타입
				...
				str+= "<li><img src='/ETS/display?fileName="+fileCallPath+"'>"+"<span data-file='"+fileCallPath+"' data-type='image'>삭제</span>"+"</li>";
			}else{// 그외 타입
				str+= "<li><a href='/ETS/download?fileName="+fileCallPath+"'>"+"<img src='resources/img/캡처.PNG'>"+attach.fileName+"</a>"+"<span data-file='"+fileCallPath+"' data-type='image'>삭제</span>"+"</li>";
			}
		});
		$(".uploadResult ul").append(str);
	});
