	...
	<form role="form">
		<table>
			...
			<tr>
				...
				<td id="title"><a href="/ETS/merboard/read?bno=${board.bno}&pageNum=${pageMaker.cri.pageNum}">${board.title}</a></td>
		...
		<div id="paging">
			<c:if test="${pageMaker.prev}">
				<a href="/ETS/merboard/list?pageNum=${pageMaker.startPage-1}&type=${pageMaker.cri.type}&keyword=${pageMaker.cri.keyword}">이전</a>
			</c:if>
			<c:forEach var="num" begin="${pageMaker.startPage}" end="${pageMaker.endPage}">
				<a href="/ETS/merboard/list?pageNum=${num}&type=${pageMaker.cri.type}&keyword=${pageMaker.cri.keyword}">${num}</a>
			</c:forEach>
			<c:if test="${pageMaker.next}">
				<a href="/ETS/merboard/list?pageNum=${pageMaker.endPage+1}&type=${pageMaker.cri.type}&keyword=${pageMaker.cri.keyword}">다음</a>
			</c:if>
		</div>
		<div id="search">
			<form action="/ETS/merboard/list" method="get">
					<select name="type">
